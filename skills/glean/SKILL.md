---
name: glean
description: Run another skill incrementally — only on the files changed since that skill last ran, tracked by the glean CLI. Use it to loop a cleanup or quality skill without re-processing the whole diff each tick (e.g. /loop 5m /glean <skill>). Works with built-in skills too.
argument-hint: "<skill>"
...
`context: fork` runs this whole skill in its own subagent, so each loop tick returns only a one-line result to the loop — the wrapped skill's instructions, file reads, and edits stay in the fork and never pile up in the loop's window. To watch or steer a pass instead, run the skill directly (`/<skill> <files>`).

Requires the `glean` binary on `PATH` (`cargo install --path . --root ~/.local`).

## Arguments

Run the skill named `$ARGUMENTS`. Drop a single leading `/` if present — the bare name is both the skill to invoke and the glean consumer (`<skill>` below). If `$ARGUMENTS` is empty, report usage and stop. It can be any skill that takes file paths as positional arguments — there is no fixed set.
...
3. **Run** — invoke the `$ARGUMENTS` skill (drop any leading `/`) through the Skill tool with `$FILES` as positional arguments and nothing else. The target skill scopes itself to those paths and does its own editing and verification.
...
- The target skill must accept positional paths. One that ignores them runs on its own default scope; the run still marks `$FILES`.
- One skill = one consumer = one baseline. Two skills looped over the same tree keep separate baselines, so each re-checks what the other edited. Run them as separate loops.
...
- `glean` tracks uncommitted work only (`git diff HEAD` + untracked); a committed change drops out by design. For committed or whole-branch work, run the skill standalone in its own branch mode (`/<skill> main`), not through `glean` — scoping by a ref is git's job.
